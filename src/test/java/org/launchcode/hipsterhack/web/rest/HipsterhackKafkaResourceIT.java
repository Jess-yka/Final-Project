...
import org.launchcode.hipsterhack.config.KafkaProperties;
...
class HipsterhackKafkaResourceIT {
    ...
    void setup() {
        ...
        HipsterhackKafkaResource kafkaResource = new HipsterhackKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/hipsterhack-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/hipsterhack-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
